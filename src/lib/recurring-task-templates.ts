...
export function stripRecurringTaskMeta(description?: string | null) {
  if (!description) {
    return "";
  }

  return description.replace(RECURRING_TASK_MARKER, "").trim();
}

export function embedRecurringTaskDescription(description?: string | null) {
  const cleaned = stripRecurringTaskMeta(description);
  return [cleaned, RECURRING_TASK_MARKER].filter(Boolean).join("\n\n").trim();
}

export function getTodayPlanDraftStorageKey(userId: string) {
...
export function appendRecurringTemplateToTodayPlan(
  ...
  const storageKey = getTodayPlanDraftStorageKey(userId);
