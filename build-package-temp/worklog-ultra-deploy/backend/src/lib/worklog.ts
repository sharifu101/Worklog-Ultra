...
export async function getAssignmentNotifications(userId: string) {
  ...
  return tasks
    ...
    .map((task) => ({
      ...
      updatedAt: task.updates[0]?.updatedAt?.toISOString() ?? new Date().toISOString(),
    }));
}
...
export async function getPendingReportEditRequests(reviewer: {
  ...
  return db.reportEditRequest.findMany({
    where: {
      status: "pending",
      dailyTask: reviewer.role === UserRole.manager && reviewer.departmentId ? { departmentId: reviewer.departmentId } : undefined,
    },
    include: {
      requestedBy: {
        include: {
...
export async function getTeamData(role: UserRole, departmentId?: string | null) {
  const today = toDateOnly();
  const sharedWhere = role === UserRole.manager && departmentId ? { departmentId } : {};
