...
function createBaseUpdates(tasks: ReportTask[]): ReportUpdateState[] {
  return tasks.map((task) => ({
    ...
    timerState:
      task.updates[0]?.status === "in_progress" && task.updates[0]?.actualStart && !task.updates[0]?.actualEnd
        ? "running"
        : "idle",
    runningStartedAt:
      task.updates[0]?.status === "in_progress" && task.updates[0]?.actualStart && !task.updates[0]?.actualEnd
        ? new Date(task.updates[0].actualStart).toISOString()
        : "",
  }));
}
      ...
      return {
        ...
        ...(sharedTimer
          ? {
              ...
              timerState:
                sharedTimer.runningStartedAt || (sharedTimer.status === "in_progress" && sharedTimer.actualStart && !sharedTimer.actualEnd)
                  ? "running"
