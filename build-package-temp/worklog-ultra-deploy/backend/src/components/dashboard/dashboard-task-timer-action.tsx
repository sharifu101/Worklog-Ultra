...
export function DashboardTaskTimerAction({
  ...
  const [runningStartedAt, setRunningStartedAt] = useState(() => {
    if (initialStatus === "in_progress" && initialActualStart && !initialActualEnd) {
      return new Date(initialActualStart).toISOString();
    }
    return "";
  });
  ...
  const liveTrackedSeconds =
    runningStartedAt && !Number.isNaN(new Date(runningStartedAt).getTime())
