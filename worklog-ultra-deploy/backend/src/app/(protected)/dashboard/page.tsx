...
import { DashboardPerformanceSpotlight } from "@/components/dashboard/dashboard-performance-spotlight";
...
function getMotivationalMessage(input: {
  ...
  const rolePools = {
    employee: {
      strong: [
        `Hey ${input.name}, you are doing great today, keep showing your best.`,
        `Hey ${input.name}, your work is shining today, stay strong.`,
        `Hey ${input.name}, you are in a great rhythm today, keep moving.`,
        `Hey ${input.name}, your best work is showing today, keep it up.`,
      ],
      steady: [
        `Hey ${input.name}, one good step now can make your whole day better.`,
        `Hey ${input.name}, keep it simple today and your progress will grow.`,
        `Hey ${input.name}, your next task can change the pace of your day.`,
        `Hey ${input.name}, small progress today still means a strong day.`,
      ],
      recovery: [
        `Hey ${input.name}, you can still turn this day around with one strong move.`,
        `Hey ${input.name}, start again now and your flow will come back.`,
        `Hey ${input.name}, do not stop today, your next step still matters.`,
        `Hey ${input.name}, you still have time to show your best today.`,
      ],
    },
    hr: {
      strong: [
        `Hey ${input.name}, your support is making the whole team stronger today.`,
        `Hey ${input.name}, your follow-up is helping everyone move better today.`,
        `Hey ${input.name}, you are holding the team together today, keep going.`,
        `Hey ${input.name}, your work is helping many people today, stay sharp.`,
      ],
      steady: [
        `Hey ${input.name}, one clear follow-up today can help the whole team.`,
        `Hey ${input.name}, your calm support can make today much smoother.`,
        `Hey ${input.name}, keep things clear today and people will feel it.`,
        `Hey ${input.name}, your steady effort is doing real work today.`,
      ],
      recovery: [
        `Hey ${input.name}, one smart follow-up now can bring your day back.`,
        `Hey ${input.name}, reset your focus and do the next helpful thing.`,
        `Hey ${input.name}, you do not need perfect, you just need one good move.`,
        `Hey ${input.name}, your next useful action can change the whole day.`,
      ],
    },
    manager: {
      strong: [
        `Hey Leader ${input.name}, you are the hero today and your team will follow you.`,
        `Hey Leader ${input.name}, your clear lead can make the whole team faster today.`,
        `Hey Leader ${input.name}, you set the pace today, so show your best.`,
        `Hey Leader ${input.name}, your team is looking at you today, lead with power.`,
      ],
      steady: [
        `Hey Leader ${input.name}, one clear direction from you can lift the whole team.`,
        `Hey Leader ${input.name}, your next smart move can guide everyone today.`,
        `Hey Leader ${input.name}, keep it clear and your team will move well.`,
        `Hey Leader ${input.name}, your simple lead can make today a strong day.`,
      ],
      recovery: [
        `Hey Leader ${input.name}, one strong decision from you can fix today fast.`,
        `Hey Leader ${input.name}, clear one blocker and your team will rise again.`,
        `Hey Leader ${input.name}, you can still turn the whole team around today.`,
        `Hey Leader ${input.name}, stay calm and lead one strong move now.`,
      ],
    },
    admin: {
      strong: [
        `Hey Leader ${input.name}, you are the hero today and the whole company can follow you.`,
        `Hey Leader ${input.name}, your next decision can move everyone faster today.`,
        `Hey Leader ${input.name}, you can unlock the best in every team today.`,
        `Hey Leader ${input.name}, your strong lead can change the whole company mood.`,
      ],
      steady: [
        `Hey Leader ${input.name}, one clear focus from you can align every team today.`,
        `Hey Leader ${input.name}, your next signal can help the whole company move.`,
        `Hey Leader ${input.name}, keep priorities clear and people will follow well.`,
        `Hey Leader ${input.name}, your leadership can make today smoother for everyone.`,
      ],
      recovery: [
        `Hey Leader ${input.name}, one sharp move from you can reset the whole company.`,
        `Hey Leader ${input.name}, your next clear decision can bring the pace back.`,
        `Hey Leader ${input.name}, remove one big blocker and everyone will breathe easier.`,
        `Hey Leader ${input.name}, you can still turn today into a strong company day.`,
      ],
    },
  } as const;
  ...
  const surprisePools = {
    employee: [
      `Hey ${input.name}, today may surprise you, so give it your best.`,
      `Hey ${input.name}, your next move can become the best part of today.`,
      `Hey ${input.name}, you are closer to a great day than you think.`,
    ],
    hr: [
      `Hey ${input.name}, one kind support move from you can lift many people today.`,
      `Hey ${input.name}, your quiet work can still change the office mood today.`,
      `Hey ${input.name}, one clean follow-up from you can save a lot of time today.`,
    ],
    manager: [
      `Hey Leader ${input.name}, today can be your day to wake the whole team up.`,
      `Hey Leader ${input.name}, your next clear call can change the whole department.`,
      `Hey Leader ${input.name}, show your best today and your team will rise with you.`,
    ],
    admin: [
      `Hey Leader ${input.name}, your next move can unlock the whole company today.`,
      `Hey Leader ${input.name}, show your best today and everyone can rise with you.`,
      `Hey Leader ${input.name}, one great decision from you can change the full day.`,
    ],
  } as const;

  const completionRate = input.plannedTasks ? input.completedTasks / input.plannedTasks : 0;
  const isStrong = completionRate >= 0.6 || input.trackedMinutes >= 180;
  const needsRecovery = input.pendingTasks >= Math.max(3, input.completedTasks + 1);
  const lane = isStrong ? "strong" : needsRecovery ? "recovery" : "steady";
  const pool = rolePools[input.role][lane];
  const dayKey = toDateOnly();
  const baseSeed = `${input.name}-${input.role}-${dayKey}-${input.plannedTasks}-${input.completedTasks}-${input.pendingTasks}-${input.trackedMinutes}`;
  const hash = buildHash(baseSeed);
  const weeklyShift = buildHash(`${input.name}-${input.role}-${dayKey.slice(0, 8)}`) % pool.length;
  const message = pool[(hash + weeklyShift) % pool.length];
  const surpriseChance = buildHash(`${baseSeed}-surprise`) % 9 === 0;
  const surprisePool = surprisePools[input.role];
  const surpriseMessage = surprisePool[buildHash(`${baseSeed}-wow`) % surprisePool.length];

  return {
    lane,
    message: surpriseChance ? surpriseMessage : message,
  };
}
...
export default async function DashboardPage() {
  ...
  const activeTasks = [...data.tasks]
    .filter((task) => task.updates[0]?.status !== "done")
  ...
  return (
    <div className="space-y-4">
      ...
      <section className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h1
            className={`max-w-5xl text-[1.28rem] font-bold leading-tight tracking-[-0.03em] md:text-[1.65rem] ${
              motivation.lane === "strong"
                ? "text-[#155e75]"
                : motivation.lane === "recovery"
                  ? "text-[#9a3412]"
                  : "text-[#1d4ed8]"
            }`}
          >
            {motivation.message.includes(user.name) ? (
              <>
                {motivation.message.split(user.name)[0]}
                <span className="bg-gradient-to-r from-[#4f5ef7] via-[#0ea5e9] to-[#10b981] bg-clip-text text-transparent">
                  {user.name}
                </span>
                {motivation.message.split(user.name).slice(1).join(user.name)}
              </>
            ) : (
              motivation.message
            )}
        ...
        <div className="flex w-full flex-wrap gap-3 md:w-auto md:justify-end xl:flex-nowrap">
          ...
          />
          <Link
            className="inline-flex shrink-0 items-center gap-2 rounded-2xl border border-[var(--panel-border)] bg-[var(--panel)] px-5 py-3 text-sm font-semibold text-[var(--foreground)] transition hover:bg-[var(--panel-alt)]"
            href="/dashboard/report"
          >
            Update Daily Report
          </Link>
      ...
      <section className="grid items-start gap-4 xl:grid-cols-[minmax(0,1.82fr)_360px]">
        <div className="space-y-4">
                            ...
                            {extractContinuationMeta(task.taskDescription) ? (
                              <div className="mt-2 flex flex-wrap gap-2">
                                ...
                                <span className="rounded-full bg-sky-50 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-sky-700">
                                  Day {extractContinuationMeta(task.taskDescription)?.daysActive || 1}
                                </span>
          ...
          {user.role === "manager" || user.role === "admin" ? (
            <DashboardPerformanceSpotlight
              members={data.performanceSpotlights.members}
              steadyPerformer={data.performanceSpotlights.steadyPerformer}
              topPerformer={data.performanceSpotlights.topPerformer}
            />
          ) : null}
